/**
 * Created by barath on 10/29/2016.
 */
...
var app = express();
var result={'body': []};
var url = 'mongodb://abhiram383:abhiram!383@ds149030.mlab.com:49030/lab10';
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('userinfo').insertOne( data, function(err, result) {
            if(err){
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('userinfo').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
                res.write("Registration Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('userinfo').deleteOne({"_id": ObjectId(id)}, function(err, result) {
